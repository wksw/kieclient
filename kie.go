...
// Create 创建配置
func (c Client) Create(project string, in *KVRequest) (*KVDoc, error) {
	...
	if err := json.Unmarshal(resp, &out); err != nil {
		return &KVDoc{}, fmt.Errorf("create config umarshal resp '%s' fail[%s]", string(resp), err.Error())
	}
	return &out, nil
}

// Update 更新配置
func (c Client) Update(project, kvID string, in *KVRequest) (*KVDoc, error) {
	...
	var out KVDoc
	if err := json.Unmarshal(resp, &out); err != nil {
		return &KVDoc{}, fmt.Errorf("update config umarshal resp '%s' fail[%s]", string(resp), err.Error())
	}
	return &out, nil
}
...
// Get 获取配置详情
func (c Client) Get(project, kvID string) (*KVDoc, error) {
	...
	var out KVDoc
	if err := json.Unmarshal(resp, &out); err != nil {
		return &KVDoc{}, fmt.Errorf("get config umarshal resp '%s' fail[%s]", string(resp), err.Error())
	}
	return &out, nil
}
...
func (c Client) GetAll(project, key, match string, revision int, labels map[string]string) (*KVResponse, error) {
	...
	var out KVResponse
	if err := json.Unmarshal(resp, &out); err != nil {
		return &KVResponse{}, fmt.Errorf("get all configs umarshal resp '%s' fail[%s]", string(resp), err.Error())
	}
	return &out, nil
}

// GetTrack 获取配置修订记录
func (c Client) GetTrack(project, kvID string) (*KVDoc, error) {
	...
	var out KVDoc
	if err := json.Unmarshal(resp, &out); err != nil {
		return &KVDoc{}, fmt.Errorf("get config '%s' revision umarshal resp '%s' fail[%s]", kvID, string(resp), err.Error())
	}
	return &out, nil
}
